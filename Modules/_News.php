...
The Class itself...
*/
class News extends \Commodities\BaseActiveModule
{
    ...
    */
    public function __construct(&$bot)
    {
        ...
        $this->bot->core("settings")
            ->create("News", "News_Del", "ADMIN", "Who should be able to delete news", "ADMIN;LEADER;MEMBER;GUEST;ANONYMOUS");
        $this->bot->core("prefs")
            ->create("News", "Logonspam", "What should news spam when logging on?", "Last_headline", "Last_headline;Link;Nothing");
        $this->bot->core("prefs")
            ->create("News", "PGjoinspam", "What should news spam when joining private group?", "Nothing", "Last_headline;Link;Nothing");
    ...
    */
    public function get_news($name)
    {
        ...
        if (!empty($result)) {
            $newsdate = gmdate(
                $this->bot->core("settings")
                    ->get("Time", "FormatString"), $result[0][1]
            );
