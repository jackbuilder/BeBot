...
The Class itself...
*/
class Callers extends \Commodities\BaseActiveModule
{
    ...
    Add a caller
    */
    public function caller_add($name)
    {
    ...
    Remove a caller
    */
    public function caller_del($name)
    {
        $name = ucfirst(strtolower($name));
        if ($name == "All") {
            $this->callers = array();
    ...
    public function clear_callers($name)
    {
        $name = ucfirst(strtolower($name));
        $this->callers = array();
