...
The Class itself...
*/
class BanManager extends \Commodities\BaseActiveModule
{
    ...
    public function add_ban($source, $user, $duration, $reason)
    {
    ...
    public function del_ban($source, $user)
    {
        $id = $this->bot->core('player')->id($user);
        $user = ucfirst(strtolower($user));
        if ($id == 0) {
