...
class AoOldChat
{
	public $state, $debug, $id, $gid, $chars, $char, $grp, $buddies;
	public $socket, $last_packet, $last_ping, $callback, $cbargs;
	
	public static $instance = array();
	/* Initialization */
/*	function __construct($cb, $game, $args = NULL)
...
*/
	...
	/* Network stuff */
	function connect($server = "default", $port = "default", $sixtyfourbit = true)
	{
	...
	*/
	function authenticateConan($username, $password, $character)
	{
		...
		if(@socket_connect($this->socket, $this->ServerAddress, $this->ServerPort) === false)
		{
			trigger_error("Could not connect to the Territory server (" . $this->ServerAddress . ":" . $this->ServerPort . "): ".
			socket_strerror(socket_last_error($this->socket)), E_USER_WARNING);
			$this->disconnect();
			return false;
		}
		// Reset this
